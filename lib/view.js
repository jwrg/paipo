...
const view = module.exports = {
  ...
  calendar: async (ctx, next) => {
    await ctx.render('calendar', {
      ...
      data: await (await db.pool.query(queries.calendar.dates(ctx.params.year, ctx.params.month))).rows
    });
  },
  /**
   * List views
  ...
  editEntry: async (ctx, next) => {
    await ctx.render('editentry', {
      id: ctx.params.id,
      data: await (await db.pool.query(queries.edit.byId(ctx.params.id))).rows
    });
  },
  /**
   * New data entry view
